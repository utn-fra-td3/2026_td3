...
#define I2S_MCLK_GPIO   GPIO_NUM_14   // → PCM1808 SCKI (via 33 ohm recomendado para integridad de señal)
...
#define FRAMES_PER_BUF   512          // frames estéreo por buffer (ajustar según latencia deseada y uso de CPU)
#define CHANNELS         2            // estéreo: 2 canales (L y R)
// Total de int32_t por buffer: 512 frames × 2 canales = 1024 muestras = 4096 bytes
...
#define LCD_I2C_ADDR     0x3F        // PCF8574A: dirección de fábrica con A0=A1=A2=1 (pull-up, sin jumpers)
...
#define STACK_AUX        2048   // stack tareas auxiliares
...
// Deben estar alineadas a 16 bytes y ser FIR_TAPS elementos.
static __attribute__((aligned(16))) float delay_lp[CHANNELS][FIR_TAPS];
static __attribute__((aligned(16))) float delay_bp[CHANNELS][FIR_TAPS];
static __attribute__((aligned(16))) float delay_hp[CHANNELS][FIR_TAPS];
...
// fir_design_band() sin cambios — genera Windowed-Sinc con ventana de Hamming
static void fir_design_band(float *coeffs, float fc_low, float fc_high, float fs)
{
    float fnl  = fc_low  / fs;
    float fnh  = fc_high / fs;
    int   half = (FIR_TAPS - 1) / 2;
    ...
    for (int n = 0; n < FIR_TAPS; n++) {
        ...
        if (x == 0.0f) {
            ...
            h = (sinf(2.0f * (float)M_PI * fnh * x) -
                 sinf(2.0f * (float)M_PI * fnl * x)) / pi_x;
        }
        float w  = 0.54f - 0.46f * cosf(2.0f * (float)M_PI * n / (FIR_TAPS - 1));
        coeffs[n] = h * w;
    }
    float sum = 0.0f;
...
{
    fir_design_band(lp_coeffs, 0.0f,    300.0f,  (float)SAMPLE_RATE); //LPF = {0 Hz - 300 Hz}
    fir_design_band(bp_coeffs, 300.0f,  3000.0f, (float)SAMPLE_RATE); //BPF = {300 Hz - 3 kHz}  
    fir_design_band(hp_coeffs, 3000.0f, 22050.0f,(float)SAMPLE_RATE); //HPF = {3 kHz - 22.05 kHz} frecuencia de Nyquist
...
static void guardian_task(void *pv)
{
    ...
    i2s_channel_register_event_callback(i2s_rx_handle, &cbs, NULL); // solo el canal RX necesita callback para sincronizar con el ADC
