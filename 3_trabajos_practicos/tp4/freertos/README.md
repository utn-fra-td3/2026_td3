...
```cmake
# Habilito FreeRTOSConfig.h de la aplicación (si el archivo existe)
set(APP_FREERTOS_CONFIG "${CMAKE_CURRENT_LIST_DIR}" CACHE PATH "Custom FreeRTOSConfig.h path")
...
Muchas de las particularidades de FreeRTOS se configuran desde el archivo [FreeRTOSConfig.h](include/FreeRTOSConfig.h). Sin embargo, si una determinada aplicación requiriera de alguna configuración particular (más memoria dinámica, acceso a alguna API en especial, memoria estática, etc), es posible pisar esa configuración creando un `FreeRTOSConfig.h en el propio directorio del proyecto. 

Este archivo debe estar en el raiz del proyecto de esta forma:
